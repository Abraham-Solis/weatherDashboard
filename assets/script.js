...
document.getElementById('search').addEventListener('click', event => {
    ...
    .then(res => {
      ...
      axios.get(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longtitude}&units=imperial&appid=d91f911bcf2c0f925fb6535547a5ddc9`)
        ...
        .then(resp => {
          ...
          <div class= "currentday" style="justify-content-center">
          <h3>City: ${city} <img src="http://openweathermap.org/img/w/${current.icon}.png" alt="icon"></h3>
            <h2>UVI: ${uvi}</h2>
            <h2>Temperature: ${current.temp}</h2>
            <h2>Humidity: ${current.humid}</h2>
            <h2>Wind: ${current.wind}</h2>
          </div>
        `
  ...
  <img class="card-img-top" src="http://openweathermap.org/img/w/${day.icon}.png" alt="Card image cap">
  <div class="card-body">
    ...
    .then(res => {
      ...
      axios.get(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longtitude}&units=imperial&appid=d91f911bcf2c0f925fb6535547a5ddc9`)
        ...
        .then(resp => {
          ...
          <div class= "currentday" style= "background-color:#89B0AE " width = 10px>
          <h3 style = "color:white">Your City: ${city}</h3>
                 
                  <h3 style = "color:white">Temperature: ${current.temp}</h3>
                  <h3 style = "color:white">Humidity: ${current.humid}</h3>
                  <h3 style = "color:white">Wind: ${current.wind}</h3>
                  <h3 style = "color:white">UVI: ${uvi}</h3>
                </div>
                </div>
              `
          ...
          forecasts.forEach(day => {
            ...
            document.getElementById('forecast').innerHTML += `
                    ...
                    <div class="card" style="width: 15rem";>
                        ...
                        <div class="card-body" style= "background-color:#BEE3DB">
                        <h5 class="card-title">${day.date}</h5>
                        <p class="card-text">            
                        <h3>temp: ${day.temp}</h3>
                        <h3>humid: ${day.humid}<h3>
